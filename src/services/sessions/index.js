...
sessionRouter.post("/:hostId", async (req, res, next) => {
  try {
    ...
    await PlayerModal.findByIdAndUpdate(req.params.hostId, {
      $push: { host: hostId },
    });
...
sessionRouter.get("/:status?", async (req, res, next) => {
  try {
    const activeStatus = req.params.status === "active";
    const sessions = await SessionModel.find({})
      .populate("host")
      .populate("players");

    const filteredSession = sessions.filter(
      (session) => session.active_game === activeStatus
    );

    res.status(200).send(filteredSession);
  } catch (error) {
    console.log("problem", error);
    next(error);
  }
