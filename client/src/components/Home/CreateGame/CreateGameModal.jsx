...
function CreateGameModal(props) {
  ...
  const isLocation = locations.find(
    (location) =>
      location.location_name === data.session_location ||
      initialState.session_location
  );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_URL}/players/me/session/${
          game ? game._id : ""
        }`,
        {
          method,
            ...
            <Row>
              <Form.Group xs={6} as={Col}>
                <Form.Control
                  ...
                  onChange={(e) =>
                    setData({ ...data, session_date: e.target.value })
                  }
                />
              </Form.Group>
              <Form.Group as={Col}>
                <Form.Control
                  ...
                  onChange={(e) =>
                    setData({ ...data, session_time: e.target.value })
                  }
                />
              </Form.Group>
            </Row>
            <Row>
                  ...
                  <span className="location-list">
                    {locations
                      .filter((item) =>
                        item.location_name
                          .toLowerCase()
                          .includes(data.session_location.toLowerCase())
                      )
                      .map((location) => (
                        <span
                          onClick={() =>
                            setData({
                              ...data,
                              session_location: location.location_name,
                            })
                          }
                          key={location._id}
              ...
              <Form.Group as={Col}>
                <Form.Control
                  ...
                  onChange={(e) =>
                    setData({ ...data, session_room: e.target.value })
                  }
                />
              </Form.Group>
            </Row>
